...
**Deploying:** on the main node, `tailscale up` (interactive login — the one manual step), then `bootstrap.sh 70` (`steps/70-dashboard.sh`, also suggested by `install.sh`): installs Tailscale + the compose plugin, runs `docker compose up -d`, and fronts it with `tailscale serve --bg --http 80 http://127.0.0.1:8090`. Manual equivalent: those last two commands. Do **not** use `serve --https 443`: it intercepts TLS for the node's tailnet IP, the same IP `dash.majksa.net` resolves to, and Tailscale has no cert for that name — it takes `dash.majksa.net` down. HTTPS (and so the PWA) comes from Caddy on `dash.majksa.net`.
