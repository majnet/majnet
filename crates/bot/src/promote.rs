...
async fn do_promote(state: &AppState, org: &str, app: &str) -> Result<String> {
    anyhow::ensure!(
        app.chars().all(|c| c.is_ascii_lowercase() || c.is_ascii_digit() || c == '-'),
        "invalid app name"
    );
    ...
    let stable = read_file(&repos, &format!("apps/{app}/stable.yaml"))
        .await?
        .with_context(|| format!("apps/{app}/stable.yaml not found — nothing deployed to stable"))?;
    let image = stable
        .0
        .lines()
        ...
        .trim()
        .to_string();
    anyhow::ensure!(image.contains("@sha256:"), "stable image is not digest-pinned: {image}");
    ...
    match read_file(&repos, &production_path).await? {
        Some((current, sha)) => {
            ...
            repos.update_file(&production_path, &message, &updated, &sha).branch("main").send().await?;
        }
        None => {
            let content = format!("# production overlay for {app} — digest moves via promote\nimage: {image}\n");
            repos.create_file(&production_path, &message, &content).branch("main").send().await?;
        }
    }
    state.store.log_event("promote", Some(org), &format!("{app} → {image}"))?;
...
async fn read_file(
    ...
    match repos.get_content().path(path).r#ref("main").send().await {
        Ok(content) => {
            ...
            let encoded = item.content.clone().unwrap_or_default().replace(['\n', ' '], "");
